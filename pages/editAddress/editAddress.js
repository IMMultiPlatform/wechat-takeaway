...
Page({
  ...
  onLoad: function (options) {
    if (options.addrId) {
      wx.showLoading()
      var params = {
        url: `/qcl-shop/address/addrInfo/${options.addrId}`,
        method: "GET",
        data: {},
        callBack: res => {
          //console.log(res)
  ...
  initCityData: function (provinceId, cityId, areaId) {
    ...
    wx.showLoading()
    var params = {
      url: "/qcl-shop/home/areaListByPid",
      data: {
        pid: provinceId
      },
      method: "GET",
  ...
  mapAddress: function() {
    ...
    let area = location.district ? location.district : ''
    let addr = location.name ? location.name : ''
  ...
  onSaveAddr: function () {
    ...
    wx.showLoading()
    var url = "/qcl-shop/address/addAddr"
    var method = "POST"
    if (ths.data.addrId != 0) {
      url = "/qcl-shop/address/updateAddr"
      method = "PUT"
    }
    //添加或修改地址
    var params = {
      ...
      data: {
        ...
        latitude: ths.data.latitude,
        longitude: ths.data.longitude
      },
      callBack: function (res) {
        wx.hideLoading()
        wx.navigateBack({
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          ...
          wx.showLoading()
          var params = {
            url: "/qcl-shop/address/deleteAddr/" + addrId,
            method: "DELETE",
            data: {},
            callBack: function (res) {
              wx.hideLoading()
